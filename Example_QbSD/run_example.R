...
series <- "SP500"
returns <- as.numeric(Data[[series]])
...
location_type <- "zero"	# or "QAR"
CAViaR_type   <- "gAS"	# or "gSAV"


# Compute in-sample estimates and OOS forecasts
results <- QbSD(returns, location=location_type, CAViaR=CAViaR_type, alpha=alpha)
...
ylim_range <- range(c(returns, VaR_in, ES_in), na.rm = TRUE)
plot(dates, returns, type = "l", col = "black", lwd = 1.0, main = paste(series, "returns with in-sample VaR and ES at", alpha * 100, "%"), xlab = "", ylab = "", ylim = ylim_range, axes = TRUE)
