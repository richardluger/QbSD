...
TableC10  <- MCS_AL_Rank[[1]]
TableC11  <- MCS_AL_Rank[[2]]
TableC12  <- MCS_AL_Rank[[3]]
for (i in 1:nm) {
  for (j in 1:nb) {
    if (TableC10[i, j]  == 0) TableC10[i, j]  <- NA  ## "NA" represents empty cells
    if (TableC11[i, j]  == 0) TableC11[i, j]  <- NA
    if (TableC12[i, j] == 0)  TableC12[i, j]  <- NA
  }
}

print("Table C10");  TableC10
print("Table C11");  TableC11
print("Table C12");  TableC12

write.table(TableC10, file = "TableC10.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC11, file = "TableC11.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC12, file = "TableC12.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)



########### Produce Tables C7,C8,C9
...
TableC7 <- MCS_QS_Rank[[1]]
TableC8 <- MCS_QS_Rank[[2]]
TableC9 <- MCS_QS_Rank[[3]]
for (i in 1:nm) {
  for (j in 1:nb) {
    if (TableC7[i, j] == 0) TableC7[i, j] <- NA  ## "NA" represents empty cells
    if (TableC8[i, j] == 0) TableC8[i, j] <- NA
    if (TableC9[i, j] == 0) TableC9[i, j] <- NA
  }
}

print("Table C7"); TableC7
print("Table C8"); TableC8
print("Table C9"); TableC9

write.table(TableC7, file = "TableC7.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC8, file = "TableC8.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC9, file = "TableC9.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
