...
TableC4  <- MCS_AL_Rank[[1]]
TableC5  <- MCS_AL_Rank[[2]]
TableC6 <- MCS_AL_Rank[[3]]
for (i in 1:nm) {
  for (j in 1:nb) {
    if (TableC4[i, j]  == 0) TableC4[i, j]  <- NA  ## "NA" represents empty cells
    if (TableC5[i, j]  == 0) TableC5[i, j]  <- NA
    if (TableC6[i, j]  == 0) TableC6[i, j] <- NA
  }
}

print("Table C4");  TableC4
print("Table C5");  TableC5
print("Table C6");  TableC6

write.table(TableC4, file = "TableC4.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC5, file = "TableC5.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC6, file = "TableC6.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)



########### Produce Tables C1,C2,C3
...
TableC1 <- MCS_QS_Rank[[1]]
TableC2 <- MCS_QS_Rank[[2]]
TableC3 <- MCS_QS_Rank[[3]]
for (i in 1:nm) {
  for (j in 1:nb) {
    if (TableC1[i, j] == 0) TableC1[i, j] <- NA  ## "NA" represents empty cells
    if (TableC2[i, j] == 0) TableC2[i, j] <- NA
    if (TableC3[i, j] == 0) TableC3[i, j] <- NA
  }
}

print("Table C1"); TableC1
print("Table C2"); TableC2
print("Table C3"); TableC3

write.table(TableC1, file = "TableC1.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC2, file = "TableC2.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
write.table(TableC3, file = "TableC3.txt", sep = "\t", quote = FALSE, na = "", col.names = NA)
